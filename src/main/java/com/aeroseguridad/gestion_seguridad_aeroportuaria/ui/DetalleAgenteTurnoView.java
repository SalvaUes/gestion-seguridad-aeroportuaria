...
import com.aeroseguridad.gestion_seguridad_aeroportuaria.ui.dialogs.GestionarPlantillasDialog;
import com.aeroseguridad.gestion_seguridad_aeroportuaria.ui.dialogs.TurnoDialog; // <-- Importamos el nuevo diálogo
...
public class DetalleAgenteTurnoView extends VerticalLayout implements BeforeEnterObserver {
    ...
    public void beforeEnter(BeforeEnterEvent event) {
        ...
        // Usamos un fetch que traiga las plantillas para evitar errores de Lazy Loading
    ...
    private HorizontalLayout buildToolbar() {
        ...
        gestionarPlantillasButton.addClickListener(e -> new GestionarPlantillasDialog(agenteActual, plantillaTurnoService, this::refreshCalendarGrid).open());
    ...
    private Component createDayCell(List<DiaDelMes> semana, DayOfWeek dayOfWeek) {
        ...
        // Si no hay turnos, la celda entera es un botón para añadir uno nuevo
        if (dia.getTurnos().isEmpty()) {
            cell.addClassName("clickable");
            cell.addClickListener(e -> openTurnoDialog(null, dia.getFecha()));
        } else {
            // Si hay turnos, cada turno es un botón para editarlo
            for (Turno turno : dia.getTurnos()) {
                Div turnoDiv = new Div(new Span(turno.getTipoTurno().toString()));
                turnoDiv.addClassName("turno-entry");
                turnoDiv.addClassName("clickable"); // Hacemos el turno clicable
                if (turno.getTipoTurno() == TipoTurno.NOCTURNO) {
                    turnoDiv.addClassName("turno-nocturno");
                } else {
                    turnoDiv.addClassName("turno-regular");
                }
                turnoDiv.addClickListener(e -> openTurnoDialog(turno, dia.getFecha()));
                cell.add(turnoDiv);
            }
        }
        return cell;
    ...
    private void openTurnoDialog(Turno turno, LocalDate fecha) {
        // Creamos una instancia del nuevo diálogo, pasándole los servicios, el agente,
        // el turno (que puede ser nulo si es nuevo), la fecha, y el callback para refrescar.
        TurnoDialog dialog = new TurnoDialog(turnoService, agenteActual, turno, fecha, (v) -> refreshCalendarGrid());
        dialog.open();
    }
    ...
    private void refreshCalendarGrid() {
        ...
        int primerDiaOffset = primerDiaDelMes.getDayOfWeek().getValue() - 1;
